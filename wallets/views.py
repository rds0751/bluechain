...
def neft(request):
    ...
    if request.method == 'POST' and 'password' in request.POST:
        ...
        mt.save()
        user_id = request.user
        subject = 'MT5 Account Generate Request from DollarVridhix'
        ...
        from_email = 'support@DollarVridhix.com'
                ...
                if verify == True:
                    if True:
                        if request.user.wallet >= amount:
                            ...
                            user_id.save()
                            model.save()
                            subject = 'MT5 Transfer Request from DollarVridhix'
                            ...
                            response = requests.request("GET", url, data=payload)
                            print(response.text)
                            # send_mail("", "Please transfer following amount to given MT5 account. <br> Name: {}, User id: {}, Email: {}, MT5 Account: {}, Amount: {}".format(user_id.name, user_id.username, user_id.email, payment_o.mt5_account, amount*0.95), "support@DollarVridhix.com", ['rds0751@gmail.com',])
                ...
                if verify == True:
                    if True:
                        if True:
                            ...
                            user_id.save()
                            model.save()
                            subject = 'DCXa Transfer Request from DollarVridhix'
                            html_message = render_to_string('account/email/ipay.html', {'name': user_id.name, 'username':user_id.username, 'email':user_id.email, 'mt5':payment_o.mt5_account, 'amount':amount*0.95, 'id': model.id})
                            plain_message = strip_tags(html_message)
                            from_email = 'support@DollarVridhix.com'
                            to = 'support@dcxa.io'
...
def direct(request):
    user = request.user
    page = request.GET.get('page', 1)
    history_list = WalletHistory.objects.filter(user_id=str(user), comment__icontains='direct').order_by('-created_at')
    paginator = Paginator(history_list, 20)
    try:
        page = int(request.GET.get('page', '1'))
    except:
...
def booster(request):
    ...
    page_range = list(paginator.page_range)[start_index:end_index]
    print(dincome,mincome,wincome)
    return render(request, 'wallets/booster.html', {'histories':histories, 'page_range': page_range, 'd': dincome, 'm': mincome, 'w': wincome})
...
def withdrawDollarVridhix(request):
    message = ''
    if request.method == "POST":
    ...
    user = request.user
    page = request.GET.get('page', 1)
    history_list = WalletHistory.objects.filter(user_id=str(user),comment__icontains='Sent to your DollarVridhix wallet address').order_by('-created_at')
    paginator = Paginator(history_list, 20)
    try:
        page = int(request.GET.get('page', '1'))
    except:
...
def DollarVridhix_verification(request):
